describe('User should be able to save art', () => {
  ...
  it('should be able to go to my favorite pieces section and be able to see saved art pieces on saved art section', () => {
    cy.wait(['@object-fetch', '@object1', '@object2', '@object3']).then(()=> {
    cy.get(".saved-art").click()
      .get('.saved-header').should('have.text', 'My Favorite Pieces')
      .get('.saved-text').should('have.text', 'There are no saved pieces')
      .get('.home-page').click()
      cy.savePieces()
      cy.get(".art-card")
  ...
  it("should be able to delete saved pieces", () => {
    cy.wait(['@object-fetch', '@object1', '@object2', '@object3']).then(()=> {
    cy.savePieces()
      cy.get(".art-card").click().should("have.length", 1)
      .get('.fave-icon').click()
      .get(".art-card").should("have.length", 0);
    })
  });
})